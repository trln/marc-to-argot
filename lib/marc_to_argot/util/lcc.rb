    ...
    module LCC
      ...
      def self.find_path(call_number)
        ...
        begin
          ...
          $stderr.puts "\tO HAI ---- #{e.message}"
          nil
        end
      end

      def self.load_root
        path = File.expand_path('../../data/shared/lcc_callnums.json', __dir__)
      ...
      def self.extract_range_xml(elem)
        label = elem.xpath('DVAL/SYN')[0].text
        children = elem.xpath('DIMENSION_NODE').collect { |x| extract_range(x) }
        bounds = [elem.xpath('DVAL/LBOUND/BOUND/@VALUE').first, elem.xpath('DVAL/UBOUND/BOUND/@VALUE').first]
